...
- `pkg/task/tool.go` — `Task` schema, validation, runtime request
- `pkg/task/sdk.go` — registers `Task` from `tools.task`
- `pkg/task/session.go` — child session IDs and nested task guard
- `pkg/task/holder.go` — binds the tool to the SDK runtime after initialization
- `internal/agent/sdk_runtime.go` — attaches `Task` tools to runtime options
- `internal/config/config.go` — `TaskToolConfig`
