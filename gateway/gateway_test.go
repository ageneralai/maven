...
import (
	...
	"github.com/ageneralai/maven/kernel/agent/postaction"
	...
	"github.com/ageneralai/maven/plugins/triggers/cron"
	"github.com/ageneralai/maven/kernel/sessionid"
	"github.com/ageneralai/maven/kernel/health"
	"github.com/ageneralai/maven/plugins/triggers/heartbeat"
...
func TestGateway_Shutdown(t *testing.T) {
	...
	chMgr := manager.New(msgBus, testLG, nil)
...
func mockRuntimeFactory(rt agent.Runtime) RuntimeFactory {
	return func(cfg *config.Config, sysPrompt string, skillRegs []api.SkillRegistration, pluginTools []tool.Tool, _ api.SessionStore, _ *slog.Logger) (agent.Runtime, error) {
		return rt, nil
	}
}
...
func TestDefaultRuntimeFactory_NoAPIKey(t *testing.T) {
	...
	_, err := DefaultRuntimeFactory(cfg, "test prompt", nil, nil, nil, testLG)
...
func TestGateway_CronRunTurn(t *testing.T) {
	...
	j, err := g.cronService().AddJob("n", cron.EverySchedule{Interval: time.Hour}, cron.Payload{Message: "test message", Deliver: false})
	if err != nil {
		t.Fatal(err)
	}
	ge := g.pipe
...
func TestGateway_CronRunTurn_WithDelivery(t *testing.T) {
	...
	j, err := g.cronService().AddJob("n", cron.EverySchedule{Interval: time.Hour}, cron.Payload{
	...
	deliver := &cron.Deliver{Pub: bus.Publisher{Bus: g.bus}, Channels: g.channelMgr, Log: g.logger}
...
func TestGateway_CronRunTurn_InvalidDeliverPayload(t *testing.T) {
	...
	j, err := g.cronService().AddJob("bad", cron.EverySchedule{Interval: time.Hour}, cron.Payload{
...
func TestGateway_CronRunTurn_RuntimeError(t *testing.T) {
	...
	j, err := g.cronService().AddJob("n", cron.EverySchedule{Interval: time.Hour}, cron.Payload{Message: "test message"})
	if err != nil {
		t.Fatal(err)
	}
