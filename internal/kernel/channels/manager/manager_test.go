...
func (m *mockManagedChannel) Capabilities() channels.CapabilitySet { return channels.CapabilitySet{} }

func TestChannelManager_Empty(t *testing.T) {
	b := bus.New(10, mgrTestLog)
	m := New(b, mgrTestLog, nil)
...
func TestChannelManager_StartAll_CtxNotCancelledAfterReturn(t *testing.T) {
	...
	m := &ChannelManager{
		channels: map[string]channels.Channel{"live": ch},
		bus:      b,
		log:      mgrTestLog,
	}
	if err := m.StartAll(context.Background()); err != nil {
